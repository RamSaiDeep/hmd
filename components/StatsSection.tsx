      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 sm:gap-8">
          {stats.map((s) => (
            <div
              key={s.label}
              className="group relative rounded-lg border border-slate-300 bg-gradient-to-br from-white to-slate-100 p-6 text-center transition-all duration-300 hover:border-blue-500/50 hover:shadow-xl hover:shadow-blue-500/20 sm:p-8"
            >
              {/* Hover accent */}
              ...
              <div className="relative z-10">
                ...
                <p className="text-sm font-medium text-slate-600 sm:text-base">{s.label}</p>
              </div>
            </div>
          ))}
        </div>
